#!/usr/bin/env bash
source <(curl -s https://raw.githubusercontent.com/imachug/win-sudo/s/aliases.sh) && ws-start-script
...
echo "source ~/bin/win-sudo/s/path.sh" | tee -a ws-files >/dev/null 2>&1
